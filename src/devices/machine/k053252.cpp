...
void k053252_device::update_screen()
{
	...
	logerror("MCFG_SCREEN_RAW_PARAMS(%d, %d, %d, %d+%d, %d, %d, %d+%d)\n",
			 clock(),
			 hc, hbp, hbp, hc - hsw - hfp - hbp,
			 vc, vbp, vbp, vc - vsw - vfp - vbp);
	...
	m_ksnotifier_cb(clock(), hbp, width, hfp, hsw, vbp, height, vfp, vsw);
}

void k053252_device::device_post_load()
{
	int hc = (m_hc & 0x3ff) + 1;
	int hfp = (m_hfp & 0x1ff);
	int hbp = (m_hbp & 0x1ff);
