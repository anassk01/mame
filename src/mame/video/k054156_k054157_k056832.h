...
#define MCFG_K054156_054156_KSNOTIFIER_CB(_cb) \
...
public:
	...
	k054156_056832_device(const machine_config &mconfig, device_type type, const char *tag, device_t *owner, uint32_t clock);
...
private:
	...
	uint16_t m_global_offx, m_global_offy;
	const uint16_t *m_global_perlayer;
	...
	template<bool gflipx> uint32_t screen_to_tile_x(int32_t x, uint32_t delta);
	template<bool gflipx>  int32_t tile_to_screen_x(uint32_t tx, uint32_t delta);
	template<bool gflipy> uint32_t screen_to_tile_y(int32_t y, uint32_t delta);
	template<bool gflipy>  int32_t tile_to_screen_y(uint32_t ty, uint32_t delta);
