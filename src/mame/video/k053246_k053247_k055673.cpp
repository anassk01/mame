...
Registers 053246/058142 (objset1):
...
DEFINE_DEVICE_TYPE(K053246_053247, k053246_053247_device, "k055247", "053246/053247 Sprite Generator Combo")
DEFINE_DEVICE_TYPE(K053246_055673, k053246_055673_device, "k055673", "053246/055673 Sprite Generator Combo")
...
k053246_055673_device::k053246_055673_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock)
	: k053246_055673_device(mconfig, K053246_055673, tag, owner, clock)
{
}

k053246_055673_device::k053246_055673_device(const machine_config &mconfig, device_type type, const char *tag, device_t *owner, uint32_t clock)
	  ...
	  m_region(*this, DEVICE_SELF)
{
	m_is_053247 = false;
	m_no_vrcbk = false;
	m_remap_cb = remap_delegate([](offs_t offset) -> offs_t { return offset; }, nullptr);
}
...
void k053246_055673_device::set_dma_remap_cb(remap_delegate cb)
{
	m_remap_cb = cb;
}
		...
		case 1: {
			...
			for(uint32_t i=0; i<0x100; i++) {
				for(uint32_t j=0; j<8; j++) {
					uint32_t srco = m_remap_cb(src);
					m_sram[dst++] = (base[srco] << 8) | base[srco+1];
					src += 2;
				}
			}
			break;
		...
		case 4: {
			...
			for(uint32_t i=0; i<0x100; i++) {
				for(uint32_t j=0; j<4; j++) {
					uint32_t srco = m_remap_cb(src);
					m_sram[dst++] = base[srco >> 2] >> 16;
					m_sram[dst++] = base[srco >> 2];
					src += 4;
				}
			}
			break;
...
static int off_for_x[][7] = {
	{ 6000000, 1, 48, 288, 16, 32, -21 }, // goku
	{ 8000000, 0, 55, 384, 33, 40, -34 }, // xexex
	{ }
};
...
static int off_for_y[][8] = {
	{ 6000000, 1, 15, 224, 17, 8, -22 }, // goku
	{ 8000000, 0, 15, 256, 12, 6, -18 }, // xexex
	{ }
};
...
void k053246_055673_device::ksnotifier_w(int clk, int hbp, int hv, int hfp, int hs, int vbp, int vv, int vfp, int vs)
{
	clk_ = clk;
	typ_ = m_is_053247 ? 0 : 1;
	hbp_ = hbp;
	hv_ = hv;
	hfp_ = hfp;
	hs_ = hs;
...
void k053246_055673_device::bitmap_update(bitmap_ind16 *bcolor, bitmap_ind16 *battr, const rectangle &cliprect)
{
	...
	int offx = (m_global_offx - m_hscr) & 0x3ff;
	int offy = (m_global_offy - m_vscr) & 0x3ff;

	gfx_element *g = gfx(0);
	uint32_t tile_base_mask = g->elements() - 1;
...
k053246_053247_device::k053246_053247_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock)
	: k053246_055673_device(mconfig, K053246_053247, tag, owner, clock)
{
	m_is_053247 = true;
	m_no_vrcbk = true;
}
