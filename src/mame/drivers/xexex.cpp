...
{
	m_tilemap_bitmap = std::make_unique<bitmap_ind16>(bitmaps[0]->width(), bitmaps[0]->height());
}
...
void xexex_state::blender_update(bitmap_ind16 **bitmaps, const rectangle &cliprect)
{
	...
	m_render_front = false;
	m_mixer->bitmap_update(bitmaps+2, cliprect);

	m_tilemap->bitmap_update(m_tilemap_bitmap.get(), cliprect, 0);
	...
	for(int y = cliprect.min_y; y <= cliprect.max_y; y++) {
		...
		const uint16_t *tc = &m_tilemap_bitmap->pix16(y, cliprect.min_x);
...
void xexex_state::mixer_init(bitmap_ind16 **bitmaps)
{
	bitmaps[k053251_device::LAYER2_ATTR]->fill(0);
}

void xexex_state::mixer_update(bitmap_ind16 **bitmaps, const rectangle &cliprect)
{
	if(m_render_front) {
		m_sprites->bitmap_update(bitmaps[k053251_device::LAYER0_COLOR],
								 bitmaps[k053251_device::LAYER0_ATTR],
								 cliprect);
		m_lvc->bitmap_update(bitmaps[k053251_device::LAYER1_COLOR],
							 bitmaps[k053251_device::LAYER1_ATTR],
							 cliprect);
	...
	if(m_render_front != m_chenmix) {
		for(int y = cliprect.min_y; y <= cliprect.max_y; y++) {
			uint16_t *tc = &bitmaps[k053251_device::LAYER2_COLOR]->pix16(y, cliprect.min_x);
			for(int x = cliprect.min_x; x <= cliprect.max_x; x++) {
				uint16_t col = *tc;
				if(col & 0x100)
					*tc &= 0x1f0;
...
void xexex_state::sprites_wiring(uint32_t output, uint16_t &color, uint16_t &attr)
{
	color = output & 0x1ff;
...
void xexex_state::parse_control2(  )
{
	...
	/* bit 9 = mix/tilemap b clear invert */
...
static MACHINE_CONFIG_START( xexex )
	...
	MCFG_SCREEN_RAW_PARAMS(XTAL_32MHz/4, 512, 55, 55+384, 289, 15, 15+256)
	...
	MCFG_K053246_053247_DMAIRQ_CB(INPUTLINE("maincpu", 5))

	MCFG_K053250_ADD("lvc", ":lvcram")

	MCFG_K053251_ADD("mixer", k053251_device::LAYER0_ATTR)
	MCFG_K053251_SET_INIT_CB(DEVICE_SELF, xexex_state, mixer_init)
	MCFG_K053251_SET_UPDATE_CB(DEVICE_SELF, xexex_state, mixer_update)
